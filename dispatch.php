...
while ($row = $result -> fetch_assoc()) {
	...
	// Using the array_push function to assign all rows to the result-set into array var cars
	array_push($cars, $car);
}
...
if ($btnDispatchClicked == true) {
	...
	// Create var numofPatrolcarDispatched and assign the no of values in patrolcarDispatched array using count function
				...
				<div class="form-group row">
					...
					<div class="col-sm-8">
						<?php 
							...
							$sql = "SELECT incident_type_id FROM incident_type WHERE incident_type_id ='" . $typeofIncident . "'";
							// Create var result to contain the result-set from SQL query
							$result = $conn -> query($sql);
							// Using a while loop to extract incident_type_id from incident_type table
							while ($row = $result -> fetch_assoc()) {
								$desc = $row['incident_type_id'];
								echo $desc;
							}
							$conn->close();
						?>
